...
class GraphTraversal:
    def __init__(self, root):
        self.window = root
        self.make_canvas = Canvas(self.window,bg="black",relief=FLAT,width=500,height=500)
    ...
    def basic_set_up(self):
        heading = Label(self.make_canvas,text="Graph Traversing",bg="black",fg="black",font=("Arial",20,"bold","italic"))
        heading.place(x=50,y=10)

        bfs_btn = Button(self.window,text="BFS",font=("Arial",15,"bold"),bg="black",fg="black",relief=FLAT,command=self.bfs_traversing)
        bfs_btn.place(x=20,y=530)

        dfs_btn = Button(window, text="DFS", font=("Arial", 15, "bold"), bg="black", fg="black", relief=FLAT, command=self.dfs_traversing)
        dfs_btn.place(x=400, y=530)

        self.status = Label(self.make_canvas,text="Not Visited",bg="black",fg="white",font=("Arial",20,"bold","italic"))
        self.status.place(x=200,y=470)
    ...
    def bfs_traversing(self):
        try:
            self.status['text'] = "white: Visited"
            ...
            while self.queue_bfs:
                ...
                print(take_vertex)
                self.make_canvas.itemconfig(take_vertex,fill="white")
                self.window.update()
                time.sleep(0.1)
